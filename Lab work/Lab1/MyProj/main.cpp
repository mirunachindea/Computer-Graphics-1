...
int main(int argc, char * argv[]) {
    ...
    rectagleColor = SDL_MapRGB(windowSurface->format, 100, 100, 100);
    ...
    while (!quit) {
        ...
        if(SDL_WaitEvent(&currentEvent) != 0)
        {
            ...
            if(currentEvent.type == SDL_KEYDOWN)
            {
                ...
                {
                    case SDLK_UP:
                        rectagleColor = SDL_MapRGB(windowSurface->format, 255, 100 , 100);
                        break;

                    case SDLK_l:
                        rectagleColor = SDL_MapRGB(windowSurface->format, 100,255, 100);
                        break;
